...
类值（class value）即对象（object），一个CData数据文件必须包含且仅包含一个根对象，每个对象必须标明它所属的类，因为在契约中，每个类型被包含在名称空间中的，在数据中，先声明名称空间的URI，然后通过别名（alias，如上例中的`a0`，`a1`）来引用该URI。`URI别名:类型名`即明确指明了一个类型。对象的属性包含在`{}`中，属性通过`属性名 = 值`来表达。数值原子值和布林原子值可以直接写出（如`1`，`123.99`，`true`），其它原子值（atom value）通过字符串来表达（如`"2015-03-31T10:26:50.4939151+08:00"`）。映射值（map value）使用`#[]`表达，其中包含键-值（`键 = 值`）对的集合，列表值（list value）和 集合值（set value）使用`[]`表达。
